  ...
  return (
    <div>
      ...
      <div className="relative w-full bg-[#0f2744]" style={{ minHeight: '420px', maxHeight: '100vh' }}>
        <img
          src={trek.coverImage || 'https://images.unsplash.com/photo-1551632811-561732d1e306?w=1600&q=80'}
          alt={trek.title}
          style={{
            display: 'block',
            width: '100%',
        ...
        <div className="absolute inset-0 flex flex-col justify-end pb-8 md:pb-12 px-4 md:px-12">
          ...
          <div className="flex flex-wrap gap-4 text-sm text-white/80">
            {trek.durationDays && <span className="flex items-center gap-1"><Calendar size={14} /> {trek.durationDays} Days</span>}
            {trek.distanceKm && <span className="flex items-center gap-1"><Map size={14} /> {trek.distanceKm} km</span>}
            {trek.maxAltitude && <span className="flex items-center gap-1"><Mountain size={14} /> {trek.maxAltitude.toLocaleString()} ft Max Altitude</span>}
            {trek.bestTime && <span className="flex items-center gap-1"><Clock size={14} /> Best: {trek.bestTime}</span>}
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            {trek.overview ? (
              <Expandable maxHeight={250}>
                <div className="text-slate-600 leading-relaxed prose prose-headings:mt-4 prose-headings:mb-2 prose-h3:mt-2 prose-p:mt-2 prose-p:mb-2 max-w-none" dangerouslySetInnerHTML={{ __html: trek.overview }} />
              </Expandable>
            ) : (
